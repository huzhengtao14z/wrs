...
if __name__ == '__main__':
    ...
    base = wd.World(cam_pos=[3,0,0], lookat_pos=[0, 0, 0])
    ...
    obj_box = cm.gen_sphere(radius=0.05, rgba=[.3, 0, 0, 1])
    obj_bd_box = bdm.BDModel(obj_box, mass=.3, type="triangles")
    obj_bd_box.set_pos(np.array([0, 0, 0.21]))
    ...
    plane1bm.set_pos(np.array([0, 0, 0]))
    ...
    planebm.set_pos(np.array([0, 0, 0.1]))
    ...
    gripper.gen_dynamicmodel(base = base)
    # base.attach_internal_update_obj(gripper.gen_dynamicmodel())
    base.run()
