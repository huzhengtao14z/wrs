...
if __name__ == '__main__':
    ...
    name =  namelist[0].split('.')[0]
    print(name)
    n = str(1)
    pcd = o3d.io.read_point_cloud('./pc_cylinder/'+name+n+".ply")
    ...
    model = load_model('fcn-com2.h5')
    pcd_list = vdda.o3dpcd_to_parray(pcd)
    # result = model.predict(np.expand_dims(pcd_list, axis=0),  verbose=1)
    # print(result)
    with open('cc_com.pickle', 'rb') as f:
        com_list = pickle.load(f)
    com = com_list[name]
    gm.gen_sphere([0.004,0.001, -0.0043], radius=0.003).attach_to(base)
    gm.gen_pointcloud(pcd_list).attach_to(base)
    gm.gen_sphere([0.005,0, -0.005], radius=0.003, rgba=(0,0,1,1)).attach_to(base)
    base.run()
