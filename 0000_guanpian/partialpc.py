...
if __name__ == '__main__':
    ...
    # name = "armadillo.stl"
    name = "guanpian_asb.stl"
    obj = tw.TrimeshHu("objects", name,scale=1)
    mesh = obj.outputTrimesh
    testmesh = gm.GeometricModel(mesh)
    testmesh.set_rgba([0, 0, 0, 0.1])
    testmesh.attach_to(base)

    Mesh = o3d.io.read_triangle_mesh('objects/guanpian_asb.stl')
    # Mesh.compute_vertex_normals()
    pcd1 = Mesh.sample_points_poisson_disk(number_of_points=10000)
    # pcd1, ind = pcd1.remove_radius_outlier(nb_points=15, radius=5)
    pcd1_np = vdda.o3dpcd_to_parray(pcd1)
    gm.GeometricModel(pcd1_np).attach_to(base)
    base.run()
    ...
    import basis.o3dhelper as oh

    # view_pc = oh.nparray2o3dpcd(viewedmesh)
    # gm.GeometricModel(view_pc).attach_to(base)

    Mesh = o3d.io.read_triangle_mesh('testtest.stl')
    # Mesh.compute_vertex_normals()
    pcd1 = Mesh.sample_points_poisson_disk(number_of_points=1000)
    # pcd1, ind = pcd1.remove_radius_outlier(nb_points=15, radius=5)
    pcd1_np = vdda.o3dpcd_to_parray(pcd1)
    gm.GeometricModel(pcd1_np).attach_to(base)
    base.run()
