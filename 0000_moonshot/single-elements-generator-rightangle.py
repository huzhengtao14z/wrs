...
class Element(object):
    def __init__(self, node, dict = "circ", radius = 0.01, manualset = False, id = None, cut = None, support = None):
        # self.construct()
        self.radius = radius
        self.id = id
    ...
    def construct(self):
        ...
        if self.support:
            if self.parity == "even-even":
                ...
                #                                           np.array([self.c3[0], self.c3[1], self.supportoffset]),
                #                                           self.radius *2.1)
                # self.pillar_b_2 = cylinder_link_start_end(self.c4,
                #                                           np.array([self.c4[0],self.c4[1], self.supportoffset]),
                #                                           self.radius *2.1)
...
if __name__ == '__main__':
    ...
    #     array = []
    #     x = np.cos(item*2*np.pi/(n-3)) * r
    #     z = np.sin(item*2*np.pi/(n-3)) * r
    ...
    for layer in range(0, height_layer):
        ...
        for item in range(0,n):
            if item <= c1:
                ...
                x = interval*c1+np.sin((item - c1) * 2 * np.pi / (c3)) * r
                z = r-(np.cos((item - c1) * 2 * np.pi / (c3)) * r)
            else:
                x = interval * c1 + np.sin((c2-c1) * 2 * np.pi / (c3)) * r
                z = r-(np.cos((c2- c1) * 2 * np.pi / (c3)) * r) + (item-c2)* interval
    ...
    # offset_matrix = [
    #     [[0, 0, 0.006 * math.sqrt(abs(4 * 4 - 0.7*(x - 4) * (x - 4) ))] for x in range(len_num)]
    ...
    # c1 = cm.gen_box(extent=[.006 * 20, 0.006 * 30, 0.006 * 30], homomat=rm.homomat_from_posrot([0, 0.006 * 4.5, 0],
    #                                                                                       rm.rotmat_from_axangle(
    #                                                                                           [0, 0, 1],
    #                                                                                           0 * np.pi / 2)),
    #                                                                                             rgba=[0, 0, 0, 0.2])
    c1 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 9, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
    ...
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], radius=0.0006, id = key, cut = cut_list, support = False)
        # element.get_stl()
