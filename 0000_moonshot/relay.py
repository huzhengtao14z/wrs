...
if __name__ == '__main__':
    ...
    class Table():
        ...
        def table_up(self, offset = 0, marker = "0", debug = False):
            '''
            Before rising, table will first down to the bottom for returing to zero
             (raising height depends on the running time)

            :param offset: time offset for fine-tune the height
            :param marker: customized offset marker
            :param debug: for checking the running time
            :return: moving table. CAUTION!!!!!!!!
            '''
            self.table_down()
            if marker == "nagado":
                offset = 4
