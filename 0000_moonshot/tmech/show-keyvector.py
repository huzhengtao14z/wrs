...
# import robot_sim.
import os
def capsule_link_start_end(start, end, radius = 0.0003, rgba = (.5,0.5,.5,0.3)):
    start = start
    end = end
    radius = radius
...
if __name__ == '__main__':
    ...
    node = Node(grid, height=0.006, origin_offset=0.000)
    ...
    for id, key in enumerate(matrix_infos.keys()):
        ...
        if id == 0:
            ...
            gm.gen_sphere(pos=element.c2, radius=0.0006, rgba=(0, 0, 1, 1)).attach_to(base)
            # gm.gen_sphere(pos=element.c3, radius=0.0006, rgba=(0, 1, 1, 1)).attach_to(base)
            gm.gen_sphere(pos=element.c4, radius=0.0006, rgba=(0, 0, 1, 1)).attach_to(base)

            gm.gen_arrow(spos=element.node['origin'], epos=element.t,  thickness=0.0004, rgba=(0.5, 0.5, 0.5, 1)).attach_to(base)
            gm.gen_arrow(spos=element.node['origin'], epos=element.b,  thickness=0.0004, rgba=(0.5, 0.5, 0.5, 1)).attach_to(base)
            gm.gen_arrow(spos=element.node['origin'], epos=element.c1, thickness=0.0004, rgba=(0.5, 0.5, 0.5, 1)).attach_to(base)
            gm.gen_arrow(spos=element.node['origin'], epos=element.c2, thickness=0.0004, rgba=(0.5, 0.5, 0.5, 1)).attach_to(base)
            gm.gen_arrow(spos=element.node['origin'], epos=element.c3, thickness=0.0004, rgba=(0.5, 0.5, 0.5, 1)).attach_to(base)
            gm.gen_arrow(spos=element.node['origin'], epos=element.c4, thickness=0.0004, rgba=(0.5, 0.5, 0.5, 1)).attach_to(base)
