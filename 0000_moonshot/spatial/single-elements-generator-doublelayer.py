...
class Element(object):
    def __init__(self, node, dict = "circ", radius = 0.01, manualset = False, id = None, cut = None, support = None,secondlayer = False):
        ...
        if cut:
            ...
            if a.is_mcdwith(objcm_list=cut) == False:
                self.parity = "space"
        if manualset:
            self.o = node["origin"]
            self.t = node["top"]
...
if __name__ == '__main__':
    ...
    matrix_2 = [[np.array([interval * x, interval * y, 2*interval]) for x in range(len_num)] for y in range(wid_num)]
    matrix_3 = [[np.array([interval * x, interval * y, 4 * interval]) for x in range(len_num)] for y in range(wid_num)]
    matrix_4 = [[np.array([interval * x, interval * y, 6 * interval]) for x in range(len_num)] for y in range(wid_num)]
    matrix_5 = [[np.array([interval * x, interval * y, 8 * interval]) for x in range(len_num)] for y in range(wid_num)]
    matrix_6 = [[np.array([interval * x, interval * y, 10 * interval]) for x in range(len_num)] for y in range(wid_num)]
    matrix_7 = [[np.array([interval * x, interval * y, 10 * interval]) for x in range(len_num)] for y in range(wid_num)]
    matrix_8 = [[np.array([interval * x, interval * y, 10 * interval]) for x in range(len_num)] for y in range(wid_num)]
    matrix_9 = [[np.array([interval * x, interval * y, 10 * interval]) for x in range(len_num)] for y in range(wid_num)]
    matrix_10 = [[np.array([interval * x, interval * y, 10 * interval]) for x in range(len_num)] for y in range(wid_num)]

    c1 = cm.gen_box(extent=[.006 * 20, 0.006 * 10, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 9, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    # base.run()
    c2 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 10, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    c3 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 11, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    c4 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 12, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    c5 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 13, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    c6 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 14, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    c7 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 15, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    c8 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 16, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    c9 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 17, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    c10 = cm.gen_box(extent=[.006 * 20, 0.006 * .1, 0.006 * 10], homomat=rm.homomat_from_posrot([-0.006 * 5, 0.006 * 18, -0.006 * 2],
                                                                                         rm.rotmat_from_axangle(
                                                                                             [0, 0, 1],
                                                                                             0 * np.pi / 2)), rgba=[0, 0, 0, 0.5])
    # cut_list = [c1,c2,c3,c4,c5,c6,c7,c8,c9,c10]
    # cut_list = [c1]
    # c1.attach_to(base)
    ...
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="1",radius=0.0006, id = key, cut = cut_list, support = False, secondlayer = True)

    grid = Grid(np.array(matrix_2), interval)
    node = Node(grid, height=0.006, origin_offset=0.001)
    matrix_infos = node.node_matrix_infos
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="2", radius=0.0006, id=key, cut=cut_list, support=False,
                          secondlayer=True)

    grid = Grid(np.array(matrix_3), interval)
    node = Node(grid, height=0.006, origin_offset=0.001)
    matrix_infos = node.node_matrix_infos
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="3", radius=0.0006, id=key, cut=cut_list, support=False,
                          secondlayer=True)

    grid = Grid(np.array(matrix_4), interval)
    node = Node(grid, height=0.006, origin_offset=0.001)
    matrix_infos = node.node_matrix_infos
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="4", radius=0.0006, id=key, cut=cut_list, support=False,
                          secondlayer=True)

    grid = Grid(np.array(matrix_5), interval)
    node = Node(grid, height=0.006, origin_offset=0.001)
    matrix_infos = node.node_matrix_infos
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="5", radius=0.0006, id=key, cut=cut_list, support=False,
                          secondlayer=True)

    grid = Grid(np.array(matrix_10), interval)
    node = Node(grid, height=0.006, origin_offset=0.001)
    matrix_infos = node.node_matrix_infos
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="10", radius=0.0006, id=key, cut=cut_list, support=False,
                          secondlayer=True)

    grid = Grid(np.array(matrix_6), interval)
    node = Node(grid, height=0.006, origin_offset=0.001)
    matrix_infos = node.node_matrix_infos
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="6", radius=0.0006, id=key, cut=cut_list, support=False,
                          secondlayer=True)

    grid = Grid(np.array(matrix_7), interval)
    node = Node(grid, height=0.006, origin_offset=0.001)
    matrix_infos = node.node_matrix_infos
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="7", radius=0.0006, id=key, cut=cut_list, support=False,
                          secondlayer=True)
    grid = Grid(np.array(matrix_8), interval)
    node = Node(grid, height=0.006, origin_offset=0.001)
    matrix_infos = node.node_matrix_infos
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="8", radius=0.0006, id=key, cut=cut_list, support=False,
                          secondlayer=True)
    grid = Grid(np.array(matrix_9), interval)
    node = Node(grid, height=0.006, origin_offset=0.001)
    matrix_infos = node.node_matrix_infos
    for key in matrix_infos.keys():
        element = Element(matrix_infos[key], dict="9", radius=0.0006, id=key, cut=cut_list, support=False,
                          secondlayer=True)
    ...
    # for key in matrix_infos.keys():
    #     element = Element(matrix_infos[key], dict="2-5-5", radius=0.0006, id=key, cut=cut_list, support=False,
    #                       secondlayer=False)
