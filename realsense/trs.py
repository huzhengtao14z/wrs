...
save_path = os.path.join(this_dir, "out", time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime()))
os.mkdir(save_path)
...
try:
    while True:
        ...
        depth_mapped_image = cv2.applyColorMap(cv2.convertScaleAbs(depth_image, alpha=0.6), cv2.COLORMAP_JET)
