...
class FreeholdContactpairs(object):
    ...
    def planHoldpairs(self, verticaljudge_lft=-np.cos(np.pi * 0.5 * 0.975),
                      verticaljudge_rgt=np.cos(np.pi * 0.5 * 0.975)):
        '''
...
if __name__=='__main__':
    ...
    taskMgr.doMethodLater(0.05, update, "addobject", extraArgs=[testNode,obj_show_node, counter], appendTask=True)
    base.run()
